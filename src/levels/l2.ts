...
/** 行是否包含硬实体（路径 / URL / 命令）。 */
function hasHardEntity(line: string): boolean {
  return (
 ...
 * 返回无法定位的取值列表；为空即通过。
 */
export function verifyHardSlots(slots: SummarySlots, original: string): readonly string[] {
  const missing: string[] = [];
  for (const value of [...slots.constraints, ...slots.artifacts, ...slots.todos]) {
    if (!original.includes(value)) missing.push(value);
  }
  return missing;
}
