...
function isSignificant(line: string): boolean {
  return (
    matchAll(line, RE_WINDOWS_PATH).length > 0 ||
 ...
 */
export function l3Coarsen(text: string, config: CompressConfig): string {
  const lines = text.split('\n');
  if (lines.length <= config.l1EdgeKeepLines * 2) return text;

  const keep = new Set<number>();
  const edge = config.l1EdgeKeepLines;
  for (let i = 0; i < edge; i++) keep.add(i);
  for (let i = Math.max(edge, lines.length - edge); i < lines.length; i++) keep.add(i);

  lines.forEach((line, i) => {
    if (isSignificant(line)) keep.add(i);
  });
