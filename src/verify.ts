...
/** 正则实体校验。 */
export function verifyEntities(original: string, summary: string, config: CompressConfig): VerifyReport {
  const entityRetain = {} as Record<EntityCategory, number>;
  ...
  for (const category of HARD_ENTITY_CATEGORIES) {
    ...
    for (const value of lost) missing.push(`${category}: ${value}`);
  }
  ...
  for (const [category, pattern] of Object.entries(SOFT_ENTITY_PATTERNS)) {
    const name = category as EntityCategory;
    ...
    if (retain < config.softEntityRetainThreshold) {
      ...
      if (action === 'reject') softPassed = false;
...
export function verifySummary(
  ...
  for (const value of [...slots.constraints, ...slots.artifacts, ...slots.todos]) {
    if (!original.includes(value)) slotMissing.push(`slot: ${value}`);
  }

  // §8.3 进阶事实问答：默认关闭，开启时每槽位一题、答案必须可在原文逐字定位
  let qaMissing: string[] = [];
  let questions: readonly SlotQuestion[] = [];
  ...
  if (allMissing.length === 0 && questions.length === 0) return report;
