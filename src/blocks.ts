...
import type { CompressConfig, TokenCounter } from './config.js';
import type { ContextState, Message, Segment, ToolBlock } from './contract.js';

/** 单条消息的 token 数。消息自带计数优先，否则用配置的计数器估算。 */
export function msgTokens(msg: Message, countTokens: TokenCounter): number {
  return msg.tokens ?? countTokens(msg.content);
}

/** 一批消息的总 token 数。 */
export function totalTokens(msgs: readonly Message[], countTokens: TokenCounter): number {
  let sum = 0;
  for (const msg of msgs) sum += msgTokens(msg, countTokens);
  return sum;
}

/** 该消息是否为发起工具调用的 assistant 消息。 */
function isToolCallMessage(msg: Message): boolean {
...
export function parseToolBlocks(msgs: readonly Message[], countTokens: TokenCounter): ToolBlock[] {
  ...
  for (const msg of msgs) {
    if (isToolCallMessage(msg)) {
      ...
      if (currentIds.length > 0) finish(false, true);
    ...
    if (isToolResultMessage(msg)) {
      ...
      if (currentIds.length > 0 && pending.has(callId)) {
        ...
        if (pending.size === 0) finish(true, false);
        continue;
      }
      // 孤儿 result：先收口当前块，再让它单独成块
      if (currentIds.length > 0) finish(false, false);
      blocks.push({
        id: `blk-${msg.id}`,
        startId: msg.id,
        endId: msg.id,
  ...
  // 收尾：仍有未返回的 result
  if (currentIds.length > 0) finish(false, pending.size > 0);

  return blocks;
}

/** 该块是否可参与压缩。不完整或畸形的块被强制 pin，任何级别都不参与。 */
export function isBlockCompressible(block: ToolBlock): boolean {
  return block.complete && !block.malformed;
}

/** 建立「消息 ID → block ID」映射，供切割与 pin 定位使用。 */
export function buildMsgBlockIndex(blocks: readonly ToolBlock[]): Map<string, string> {
  const index = new Map<string, string>();
  for (const block of blocks) {
    for (const id of block.msgIds) index.set(id, block.id);
  }
  return index;
}
 ...
 */
export function selectSegment(state: ContextState, config: CompressConfig): Segment[] {
  ...
  const blockHasPin = (block: ToolBlock): boolean => block.msgIds.some((id) => pinnedMsgIds.has(id));

  // tail 起点
  const tailStart = Math.max(headEnd + 1, msgs.length - config.tailTurns);

  if (tailStart <= headEnd + 1) return [];

  const segments: Segment[] = [];
  let current: ToolBlock[] = [];

  const flush = (): void => {
    if (current.length === 0) return;
    const msgIds: string[] = [];
    for (const block of current) msgIds.push(...block.msgIds);
    segments.push({
...
export function segmentsAreSafe(segments: readonly Segment[], blocks: readonly ToolBlock[]): boolean {
  ...
  for (const seg of segments) {
    ...
    if (!isBlockCompressible(first) || !isBlockCompressible(last)) return false;
  }
  return true;
}
