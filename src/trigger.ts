...
export interface TriggerContext {
  ...
  /** 距上次成功压缩经历的对话轮数（无历史记 Infinity）。 */
  readonly turnsSinceLastCompress: number;
  /** 上一轮是否刚完成压缩（迟滞带与「压缩后落定」判定用）。 */
  readonly justCompressed: boolean;
}
 ...
 */
export function detectBoundaries(state: ContextState, config: CompressConfig, space?: SimilaritySpace): BoundaryHit[] {
  ...
  for (const msg of msgs) {
    ...
    if (hasPending) seenPending = true;
  ...
  for (let i = 0; i < msgs.length - 1; i++) {
    ...
    if (!DELIVERY_KEYWORDS.some((kw) => msg.content.toLowerCase().includes(kw))) continue;
    const next = msgs[i + 1]!;
    if (next.role === 'user') {
 ...
 */
export function shouldCompress(ctx: TriggerContext, space?: SimilaritySpace): TriggerDecision {
