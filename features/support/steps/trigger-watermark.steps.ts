...
function evaluateTrigger(this: CompressWorld): void {
  const state = this.state!;
  const space = createVectorSpace();
  for (const m of state.msgs) {
    if (m.role === 'user') space.addDocument(m.content);
  }
...
Given(/压缩配置注入为 触发线 (\S+) 目标线 (\S+) 频率下限轮数 (\S+)/, function (this: CompressWorld, trigger: string, target: string, minGap: string) {
  // Background 中的尖括号为占位符（Background 不参与 Outline 替换），在此注入标定值
  const map = (raw: string, fallback: number): number => {
...
Given('上下文窗口大小已知', function (this: CompressWorld) {
...
Given('当前 token 占用率低于触发线', function (this: CompressWorld) {
  this.state = this.buildState(this.msgs);
  this.state = { ...this.state, capacity: this.capacityForRatio(0.3) };
});

When('调用 should_compress', function (this: CompressWorld) {
  evaluateTrigger.call(this);
});

Then('返回不压缩决策', function (this: CompressWorld) {
  assert.equal(this.decision?.compress, false);
});

Then('epoch 保持不变', function (this: CompressWorld) {
  assert.equal(this.decision?.epoch, this.state?.epoch);
});

Then('不调用任何压缩钩子', function (this: CompressWorld) {
  assert.equal(this.compressCalls, 0);
});

/* ---- Scenario: 占用落在迟滞带内不压缩以防抖动 ---- */

Given('当前 token 占用率高于目标线但低于触发线', function (this: CompressWorld) {
  const mid = (this.config.triggerRatio + this.config.targetRatio) / 2;
  this.state = this.buildState(this.msgs);
  this.state = { ...this.state, capacity: this.capacityForRatio(mid) };
});

Given('上一轮压缩刚刚完成', function (this: CompressWorld) {
  this.justCompressed = true;
});

Then('决策原因记为 hysteresis-band', function (this: CompressWorld) {
  assert.equal(this.decision?.reason, 'hysteresis-band');
});

Then('本轮不产生新的 CompressedBlock', function (this: CompressWorld) {
  assert.equal(this.compressCalls, 0);
});

/* ---- Scenario Outline: 识别到任务边界则在边界处压缩 ---- */

Given('当前 token 占用率高于触发线', function (this: CompressWorld) {
  this.state = this.buildState(this.msgs);
  this.state = { ...this.state, capacity: this.capacityForRatio(0.9) };
});

Given(/会话中存在 (.+) 标记的位置/, function (this: CompressWorld, signal: string) {
  let msgs: Message[];
  switch (signal) {
...
Given('等待轮数已超过配置的等待上限', function (this: CompressWorld) {
  this.turnsSinceLastCompress = this.config.maxWaitTurns + 1;
});

Then('返回压缩决策', function (this: CompressWorld) {
  assert.equal(this.decision?.compress, true);
});

Then('决策标记 forced 为 true', function (this: CompressWorld) {
  assert.equal(this.decision?.forced, true);
});

Then('切点仍受 tool block 边界门禁约束，不落在任何 block 内部', function (this: CompressWorld) {
...
Given('一轮压缩已提交且压缩后占用率低于目标线', async function (this: CompressWorld) {
...
When('紧接着再次调用 should_compress', function (this: CompressWorld) {
  evaluateTrigger.call(this);
});

Then('本轮不追加第二次压缩', function (this: CompressWorld) {
  // firstRunCalls 已在首轮提交后采样：再次判定不得新增任何钩子调用
  assert.equal(this.compressCalls, this.firstRunCalls);
});

Then('epoch 相对压缩前仅递增 1', function (this: CompressWorld) {
  assert.equal(this.lastCommitted?.epoch, this.lastEpochBefore + 1);
});

/* ---- Scenario: 频率下限内抑制压缩即使占用已超线 ---- */

Given('距上次成功压缩的轮数小于配置的频率下限轮数', function (this: CompressWorld) {
  this.turnsSinceLastCompress = 1;
});
...
Given('会话头部含系统提示与已 pin 的内容', function (this: CompressWorld) {
  const msgs = new HistoryBuilder()
    .system()
    .user('记住：输出必须使用中文注释')
    .user('开始任务')
...
When('完成一轮压缩并提交', async function (this: CompressWorld) {
  this.hookOverrides.compress = auditedGoodCompress(this);
  this.hookOverrides.shouldCompress = () => true;
  this.lastEpochBefore = this.state!.epoch;
  this.firstRunCalls = this.compressCalls;
...
Then('头部消息序列在压缩前后逐字节相同', function (this: CompressWorld) {
...
Then('cache_control 断点仍位于头部之后、首个压缩块之前', function (this: CompressWorld) {
...
Then('报告字段 cache_impact.prefix_stable 为 true', function (this: CompressWorld) {
...
Given(/^一段内容已按幂等键 \(startId, endId, level, epoch\) 生成过压缩结果$/, async function (this: CompressWorld) {
  this.hookOverrides.compress = auditedGoodCompress(this);
  this.hookOverrides.shouldCompress = () => true;
  this.state = this.buildState(this.msgs);
  this.state = { ...this.state, capacity: this.capacityForRatio(0.9) };
...
When('以完全相同的幂等键再次请求压缩', async function (this: CompressWorld) {
  this.lastCommitted = await this.orchestrator({
    compress: this.hookOverrides.compress!,
    shouldCompress: this.hookOverrides.shouldCompress,
  }).maybeCompress(this.state!);
...
Then('返回缓存的既有结果', function (this: CompressWorld) {
  assert.notEqual(this.lastCommitted, this.state);
});

Then('不再调用 LLM 摘要钩子', function (this: CompressWorld) {
  assert.equal(this.compressCalls, this.firstRunCalls);
});

Then('压缩块文本与首次生成时逐字节相同以避免缓存抖动', function (this: CompressWorld) {
  assert.deepEqual(
    (this.lastCommitted?.compressed ?? []).map((b) => b.text),
    (this.firstBlocks ?? []).map((b) => b.text),
  );
});

/* ---- Scenario: 单轮内占用反复穿越触发线只压缩一次 ---- */

Given('一轮对话中 token 占用率先超触发线、压缩后回落、随后再次超线', function (this: CompressWorld) {
  this.state = this.buildState(this.msgs);
  this.state = { ...this.state, capacity: this.capacityForRatio(0.9) };
  this.turnsSinceLastCompress = this.config.maxWaitTurns + 1;
...
When('该轮内连续多次调用 should_compress', function (this: CompressWorld) {
  // 第 1 次：超线（压缩发生）
  evaluateTrigger.call(this);
...
Then('仅首次超线触发压缩', function (this: CompressWorld) {
  const log = this.decisionLog ?? [];
  assert.equal(log[0]?.compress, true);
});

Then('后续调用因频率下限被抑制', function (this: CompressWorld) {
  const log = this.decisionLog ?? [];
  assert.equal(log[1]?.reason, 'rate-limit');
  assert.equal(log[2]?.reason, 'rate-limit');
});

Then('全轮 epoch 递增次数为 1', function (this: CompressWorld) {
