...
function laterText(msgs, fromIndex) {
    return msgs
        .slice(fromIndex + 1)
...
function referencedLater(row, msgs) {
    if (row.msgIndex >= msgs.length - 1)
        return false;
 ...
 */
export function l1Denoise(msgs, config) {
    const rules = [];
    const note = (rule) => {
    ...
    if (rows.some((r) => r.rule === 'superseded-result'))
        note('superseded-result');
    ...
    for (const indices of byFp.values()) {
        ...
        if (msgIndices.size < 2)
            continue;
        const latestMsgIndex = Math.max(...msgIndices);
        for (const i of indices) {
            const row = rows[i];
            if (row.msgIndex === latestMsgIndex)
                continue;
            if (!row.keep)
                continue; // 已被更高优先级规则（superseded-result）处置
            if (referencedLater(row, msgs)) {
                row.rule = 'referenced-later';
                continue;
            }
    ...
    for (const [msgIndex, lineCount] of msgLineCount) {
        ...
        for (const i of indices) {
            const row = rows[i];
            if (head.includes(i) || tail.includes(i))
                continue;
            if (referencedLater(row, msgs)) {
                row.rule = 'referenced-later';
                continue;
    ...
    if (rows.some((r) => r.rule === 'oversized-stdout'))
        note('oversized-stdout');
    ...
    flushRun();
    if (rows.some((r) => r.rule === 'repeated-failure'))
        note('repeated-failure');
    if (rows.some((r) => r.rule === 'referenced-later'))
        note('referenced-later');
