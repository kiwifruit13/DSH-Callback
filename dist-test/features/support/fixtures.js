...
export class HistoryBuilder {
    ...
    system(text = '你是一个代码助手，遵守用户的全部约束。') {
    ...
    toolRound(name, resultText, callId) {
...
export function entityDroppingCompress() {
    return async (input) => {
...
export function firstBadThenGoodCompress() {
    let call = 0;
...
export function hangingCompress() {
    return (input) => new Promise((_resolve, reject) => {
        input.signal.addEventListener('abort', () => reject(new Error('aborted by timeout')), { once: true });
    });
}
/** L2+ 输出缺少 slots（不符合 schema）→ 触发重试一次再降级。 */
export function schemaInvalidCompress() {
    return async (input) => ({ text: `纯文本摘要：${input.text.slice(0, 20)}` });
}
/** 始终抛异常。三级全失败 → NoCompressionPossible。 */
export function throwingCompress() {
    return async () => {
...
export function makeState(msgs, config, opts = {}) {
...
export function segmentsFromBlocks(blocks) {
