...
async function runFullCompress() {
    this.resetRoundState();
    const state = this.state ?? this.buildState(this.msgs);
...
When('执行 on_pre_compress', function () {
...
export function runAssignBudget(world) {
    const anchor = world.anchorOverride ?? world.pins.map((p) => p.text).join('\n');
    const space = world.config.embeddingEnabled && world.config.embed !== null
    ...
    world.assignments = assignBudget({ segments: world.segments, anchorText: anchor, segmentTexts: texts, retainedTexts: world.retainedTexts, space }, world.config);
}
When('执行预算分配', function () {
    runAssignBudget(this);
});
Then('epoch 不递增', function () {
    assert.equal(this.lastCommitted?.epoch, this.lastEpochBefore);
});
Then('上下文保持原样', function () {
    assert.ok(this.lastCommitted !== null && this.state !== null);
    assert.equal(this.lastCommitted, this.state);
});
Then('上下文保持逐字节原样不产生任何 CompressedBlock', function () {
    // 引用相等 ⇒ msgs / compressed 全部保持压缩前状态（§9.2 整体替换提交的逆否）
    assert.equal(this.lastCommitted, this.state);
});
/** 最后一个非 L4 指针（method !== 'none'）的压缩块；降级链断言只针对实体压缩块。 */
export function lastRealBlock(world) {
...
Then('该块的 method 记为 heuristic', function () {
    assert.equal(lastRealBlock(this).method, 'heuristic');
});
Then('该块的 method 记为 truncate', function () {
    assert.equal(lastRealBlock(this).method, 'truncate');
});
Then('抛出 ArchiveCorrupted', function () {
...
export function countingCompress(world, impl) {
...
export function auditedGoodCompress(world) {
    return countingCompress(world, goodCompress());
}
/** 断言全部 pin_reason 属于枚举集合。 */
export function assertReasonsInEnum(world) {
    const valid = new Set(PIN_REASON_PRIORITY);
