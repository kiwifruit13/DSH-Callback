...
import { CommitAssertFailed } from '../../../src/contract.js';
...
Given('注入的 compress 钩子在处理第二个段时抛出异常', function () {
    ...
    this.hookOverrides.compress = async (input) => {
        ...
        if (call >= 1)
            throw new Error('segment two exploded');
        return { text: input.text };
    };
    this.hookOverrides.shouldCompress = () => true;
});
Given('第一个段已成功产出压缩结果', function () {
    // 由上一个 Given 的 stub 保证：第一段正常返回
});
Then('已产出的第一个段结果被丢弃不提交', function () {
    assert.equal(this.lastCommitted, this.state);
    assert.equal(this.state?.compressed.length, 0);
});
Then('ContextState 引用与 msgs 内容逐字节保持压缩前的状态', function () {
    assert.equal(this.lastCommitted, this.state);
});
Then('不存在任何部分写入的 CompressedBlock', function () {
    assert.equal(this.lastCommitted?.compressed.length, 0);
});
/* ---- Scenario: 提交前断言失败则整体回滚 ---- */
Given('待提交的 next_msgs 中某个 tool block 的配对不完整', function () {
...
When('以同一幂等键再次请求压缩', async function () {
...
Given('一轮压缩已成功提交', async function () {
    this.hookOverrides.compress = auditedGoodCompress(this);
    this.hookOverrides.shouldCompress = () => true;
    this.state = this.buildState(atomicSession());
    this.lastEpochBefore = this.state.epoch;
...
When('二者并发进入压缩流程', async function () {
...
Given('next_msgs 的实际 token 总数与压缩前预估值的偏差超出容许范围', function () {
    this.tokenDeviationCase = true;
    this.nextMsgs = new HistoryBuilder().system().user('short').build();
    this.state = this.state ?? this.buildState(atomicSession());
});
Then('记录预估偏差值以便修正 token 估算器', function () {
    const error = this.lastError;
...
When('检查其观测记录', function () {
    // 断言在 Then 中完成
});
Then('记录包含前后 token、压缩比、耗时、级别、method、pin 条数与 cache 影响字段', function () {
...
Then('观测接口以回调或事件形式暴露，不绑定任何特定监控实现', function () {
    // 记录经 config.onObservation 回调进入 world —— 回调式接口已被本场景证明
    assert.ok(this.observations.length > 0);
});
Then('记录中的 method 与 degraded 字段可区分本轮是否走了降级', function () {
    const record = this.observations[this.observations.length - 1];
    assert.ok(typeof record.degraded === 'boolean');
});
//# sourceMappingURL=atomicity-idempotency.steps.js.map