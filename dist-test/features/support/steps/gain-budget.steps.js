...
Then('该段获得充足预算且 target_level 为 1', function () {
    const a = this.assignments.find((x) => x.segmentId === 'seg-a');
...
Given('某段的报错文本与已保留段中的报错文本高度重复', function () {
    this.retainedTexts = [SEGB_TEXT];
});
Then('该段冗余度项显著抬高', function () {
    const b = this.assignments.find((x) => x.segmentId === 'seg-b');
    assert.ok(b && b.redundancy > 0.5, `冗余度 ${b?.redundancy}`);
});
Then('该段增益因 λ 加权而被压低', function () {
    const b = this.assignments.find((x) => x.segmentId === 'seg-b');
    assert.ok(b && b.gain < b.relevance, '增益应低于原始相关性');
});
Then('该段获得的预算低于内容不重复的同类段', function () {
    const a = this.assignments.find((x) => x.segmentId === 'seg-a');
    const b = this.assignments.find((x) => x.segmentId === 'seg-b');
    assert.ok(a && b);
...
Given('某段与 anchor 相关性极低且冗余度极高，增益为全部段中最低', function () {
    this.retainedTexts = [SEGB_TEXT];
});
When('执行预算分配并完成压缩', async function () {
...
Then('该段 target_level 为 4', function () {
    const b = this.assignments.find((x) => x.segmentId === 'seg-b');
    assert.equal(b?.targetLevel, 4);
});
Then('该段仍生成一个 CompressedBlock，未被丢弃', function () {
...
Then('该块的 archive_ref 非空且可成功 rehydrate 取回 L0 原文', async function () {
...
Given('配置中 embedding 开关为关闭', function () {
    this.configOverrides = { ...this.configOverrides, embeddingEnabled: false, embed: null };
    this.makeConfig();
});
Then('相关性项与冗余度项均由 TF-IDF 字符二元组向量计算', function () {
...
Then('流程不抛异常且不产生任何网络调用', function () {
    // TF-IDF 为纯本地计算：无 fetch/网络导入；不抛异常即达此处
    assert.ok(true);
});
Then('增益排序结果与开启 embedding 时同为确定值', function () {
...
Given('一段内容为会话早期的用户硬约束，与当前查询相关性低', function () {
    // prepSegments 默认：segA = 硬约束文本
});
When('以 pin 约束集为 anchor 执行预算分配', function () {
    this.anchorOverride = ANCHOR_TEXT;
    this.retainedTexts = [];
    runAssignBudget(this);
});
Then('该段增益处于高档且 target_level 为 1', function () {
    const a = this.assignments.find((x) => x.segmentId === 'seg-a');
    const b = this.assignments.find((x) => x.segmentId === 'seg-b');
    assert.ok(a && b);
...
When('改以当前用户查询为 anchor 重新执行预算分配', function () {
    this.anchorOverride = QUERY_TEXT;
    this.retainedTexts = [];
    runAssignBudget(this);
});
Then('该段增益显著下降且 target_level 大于 1', function () {
    const a = this.assignments.find((x) => x.segmentId === 'seg-a');
    const b = this.assignments.find((x) => x.segmentId === 'seg-b');
    assert.ok(a && b);
...
Then('该对照结果被记录为反例，说明为何本方案不采用当前查询作 anchor', function () {
...
Given('某段同时包含中文句子、英文标识符与文件路径', function () {
    this.mixedText = '解析用户会话中的上下文\nconst config = loadConfig()\nD:\\data\\report.docx';
});
When('计算该段的 TF-IDF 向量', function () {
    this.mixedTokens = splitTokens(this.mixedText);
});
Then('中文部分以字符二元组切分，拉丁部分以空白与标点切分', function () {
...
Then('分配结果按段的原始先后顺序确定，不随排序实现波动', function () {
...
Then('对同一输入重复执行两次得到逐字节相同的分配结果', function () {
...
Given('某段已处于 L2 且需评估是否继续下沉', function () {
    this.msgs = twoSegmentSession();
    prepSegments.call(this);
