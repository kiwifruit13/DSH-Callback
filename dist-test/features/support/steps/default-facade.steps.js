...
/* ---- 背景 ---- */
Given('归档使用内存 sink', function () {
    this.useMemoryArchive();
});
Given('注入的 callback 只实现了 compress，其余钩子全部缺省', function () {
...
Given('会话为系统提示、两条用户消息与两个完整 tool block', function () {
...
Given('会话容量使占用率高于触发线', function () {
    const state = this.state;
    const ratio = state.tokens / state.capacity;
    assert.ok(ratio > this.config.triggerRatio, `占用率 ${ratio.toFixed(3)} 应高于触发线 ${this.config.triggerRatio}`);
});
/* ---- 场景: 默认触发路径下压缩不会在首次成功后永久卡死（P0-1 回归） ---- */
Then('第一轮压缩成功提交', function () {
...
Given('在提交结果上追加新的用户消息与完整 tool block 构造下一轮状态', function () {
    const committed = this.lastCommitted;
...
Then('这两次调用均被频率下限抑制而不压缩', function () {
    const state = this.state;
...
When('继续调用一次压缩', async function () {
    this.lastCommitted = await this.pendingCompressor.maybeCompress(this.state);
});
Then('本轮压缩成功提交', function () {
    const committed = this.lastCommitted;
    assert.ok(committed !== null && committed !== this.state, '本轮应提交新状态');
    assert.equal(committed.epoch, (this.state?.epoch ?? 0) + 1);
});
Then('压缩块总数在原有基础上继续增长', function () {
    assert.ok(this.facadeCompressedAfterRound1 !== null);
    assert.ok((this.lastCommitted?.compressed.length ?? 0) > this.facadeCompressedAfterRound1, `压缩块总数应大于第一轮的 ${this.facadeCompressedAfterRound1}`);
});
Then('全程未出现钩子缺失或钩子异常错误', function () {
    assert.equal(this.lastError, null);
    assert.ok(!this.warnings.some((w) => w.includes('hook_error')), `不应出现钩子异常告警，实际：${this.warnings.join(', ')}`);
});
/* ---- 场景: 压缩后占用回落到目标线以下时不再压缩（P0-2 回归） ---- */
Given('把提交结果的容量放大使占用率回落到目标线以下', function () {
    const committed = this.lastCommitted;
    assert.ok(committed !== null, '第一轮应已提交');
    const capacity = Math.max(1, committed.tokens * 10);
    this.state = { ...committed, capacity };
    assert.ok(committed.tokens / capacity < this.config.targetRatio, `占用率应回落到目标线 ${this.config.targetRatio} 以下`);
});
Then('本轮不压缩且状态逐字节保持原样', function () {
...
Given('compress 钩子返回约束槽位含原文中不存在的取值', function () {
    ...
    this.hookOverrides.compress = countingCompress(this, async (input) => {
...
Given('会话改用含重复 hash 指纹的读取输出使 L1 降级输出可被去重缩小', function () {
...
Given('目标预算压到整段一半以下使目标段走 L2 及以上的 LLM 路径', function () {
...
Then('观测告警双通道同步记录了校验重试与降级链告警', function () {
...
Given('compress 钩子在被调用时触发外部取消并返回合法输出', function () {
    ...
    this.facadeBaselineObservations = this.observations.length;
    this.hookOverrides.compress = countingCompress(this, async (input, signal) => {
...
Then('提交前被取消：状态逐字节保持原样且不产生观测记录', function () {
    assert.equal(this.lastCommitted, this.state, '被取消的轮次必须返回原状态引用');
    assert.equal(this.observations.length, this.facadeBaselineObservations, '被取消的轮次不得产生观测记录');
});
Then('compress 钩子收到了已中止的信号', function () {
    assert.equal(this.facadeHookSawAbort, true, 'compress 钩子的 input.signal 应已中止');
});
/* ---- 场景: 布尔触发的决策原因如实标注（R5-8 回归） ---- */
Given('注入恒返回布尔真值的 shouldCompress', function () {
...
Then('观测记录的触发原因标注为宿主决策而非任务边界', function () {
    const record = this.observations[this.observations.length - 1];
    assert.ok(record !== undefined, '应存在本轮观测记录');
...
Then('观测记录确认头部前缀稳定且断点可置于头部之后', function () {
    const record = this.observations[this.observations.length - 1];
    assert.ok(record !== undefined, '应存在本轮观测记录');
...
Given('compress 钩子返回不符合槽位 schema 的输出', function () {
...
Then('观测告警包含降级链的非法 schema 记录', function () {
    const record = this.observations[this.observations.length - 1];
    assert.ok(record !== undefined, '应存在本轮观测记录');
    assert.ok(record.warnings.some((w) => w.startsWith('llm_invalid_schema_attempt')), `观测告警应含 fallback 链的 llm_invalid_schema_attempt_N，实际：${record.warnings.join(', ')}`);
    const block = lastNonPointerBlock.call(this);
    assert.ok(block.method === 'heuristic' || block.method === 'truncate', `非法 schema 应降级为抽取式或截断，实际 ${block.method}`);
});
//# sourceMappingURL=default-facade.steps.js.map