...
/* ---- Background ---- */
Given('一个已解析出头部、中部与尾部的会话', function () {
...
Given('pin 白名单识别器已启用', function () {
    // 静态白名单始终生效；本步骤确保配置已就绪
    this.makeConfig();
});
/** 在会话早期插入含指定文本的用户消息并登记受保护消息 ID。 */
function seedProtectedMsg(text) {
...
/* ---- Scenario: 用户硬约束逐字节不变 ---- */
Given('会话早期用户消息含文本 {string}', function (text) {
    seedProtectedMsg.call(this, text);
});
When('执行任意级别的压缩', function () {
...
Given('一条消息含 Windows 绝对路径 {string}', function (winPath) {
    seedProtectedMsg.call(this, `结果保存到 ${winPath}`);
    this.winPath = winPath;
});
Then('该路径在压缩结果中逐字符存在', function () {
    const joined = this.lastCommitted?.msgs.map((m) => m.content).join('\n') ?? '';
    assert.ok(joined.includes(this.winPath));
});
Then('路径类实体保留率为 1.0', function () {
...
/* ---- Scenario: 权限确认记录可被追溯 ---- */
Given('会话中存在用户对高风险操作的批准记录', function () {
    seedProtectedMsg.call(this, '我批准执行删除操作，可以执行。');
});
Then('该记录被 pin 且 pin_reason 为 permission-grant', function () {
    assert.ok(this.lastCommitted?.pins.some((p) => p.reason === 'permission-grant'));
});
Then('依据压缩后的上下文仍能判定该操作已获批准', function () {
    const joined = this.lastCommitted?.msgs.map((m) => m.content).join('\n') ?? '';
    assert.ok(joined.includes('批准'));
});
/* ---- Scenario: 未完成待办 pin，已完成待办可下沉 ---- */
Given('存在一条状态为 pending 的待办与一条状态为 completed 的待办', function () {
    const msgs = new HistoryBuilder()
        .system()
        .user('- [ ] 待办：修复登录问题\n- [x] 待办：整理文档')
...
Then('该段不占用中部预算且 target_level 恒为 0（逐字保留）', function () {
...
Then(/该内容被 pin 且 pin_reason 为 (.+)$/, function (reason) {
    assert.ok(this.pins.some((p) => p.msgId === this.pinnedMsgId && p.reason === reason), `期望 ${reason}，实际：${JSON.stringify(this.pins)}`);
});
/* ---- Scenario: 仅 pin 消息内的关键片段而非整条长消息 ---- */
Given('一条超长用户消息，其中仅有一句包含交付物绝对路径', function () {
    const filler = Array.from({ length: 12 }, (_, i) => `填充句 ${i + 1}：这里没有任何关键信息。`).join('\n');
    const text = `${filler}\n交付物输出到 D:\\out\\final-report.docx`;
    seedProtectedMsg.call(this, text);
});
Then('该消息产生的 pin 携带 span 字段指向含路径的字符区间', function () {
...
Then('该消息中 span 之外的内容仍可下沉至 L2 或更低级别', function () {
