...
import { HistoryBuilder, entityDroppingCompress, firstBadThenGoodCompress } from '../fixtures.js';
...
Given('压缩摘要中缺失了其中至少一个', function () {
    // summaryText 已在上一步构造为缺失一个实体的版本
});
When('执行实体校验', function () {
...
Then('verify_passed 为 false', function () {
    if (this.report)
        assert.equal(this.report.passed, false);
});
Then(/entity_retain 中 (.+) 的保留率小于 1\.0/, function (category) {
    const key = (CATEGORY_OF[category] ?? this.entityCategory);
    assert.ok(this.report, '应有校验报告');
    assert.ok(this.report.entityRetain[key] < 1.0, `保留率 ${this.report.entityRetain[key]}`);
});
Then('报告标记缺失的具体实体值以便定位', function () {
    assert.ok(this.report, '应有校验报告');
...
Then('抽取结果的每个实体保留率为 1.0（因输出是原文子集）', function () {
    const block = blockUnderTest.call(this);
    const original = segmentOriginalOf.call(this);
...
Given('降级为抽取式后其 token 数仍超出该段预算', function () {
...
Then('执行硬截断并保留尾部内容', function () {
    const block = blockUnderTest.call(this);
    assert.ok(block.text.length > 0);
});
Then('截断后仍对保留部分重跑一次实体校验', function () {
...
Then('本轮判定为校验不通过，不放行该摘要', function () {
    const block = blockUnderTest.call(this);
    const original = segmentOriginalOf.call(this);
...
//# sourceMappingURL=entity-verify.steps.js.map