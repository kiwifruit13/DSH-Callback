...
Given('相邻两条用户消息围绕同一主题高度相关', function () {
    const msgs = new HistoryBuilder()
        .system()
        .user('讨论数据库索引优化方案')
...
Given('相邻两条用户消息分属完全无关的话题', function () {
...
When('直接调用公共导出的 defaultShouldCompress', function () {
...
Then('不得因 topic-shift 假阳性触发压缩', function () {
    const d = this.decision;
    assert.ok(!(d.compress === true && d.boundaryType === 'topic-shift'), `相似相邻消息被误报 topic-shift（空语料空间回归）：${JSON.stringify(d)}`);
});
