...
import { HistoryBuilder, goodCompress, hangingCompress, schemaInvalidCompress, throwingCompress, } from '../fixtures.js';
...
/* ---- Background ---- */
Given('降级链按 llm → heuristic → truncate → 放弃 的顺序装配', function () {
    // 链的装配在 fallback.ts 中固定；本步骤确认配置就绪
    this.makeConfig();
});
Given('每级超时时长由配置注入且以 AbortController 控制', function () {
...
Given('注入的 LLM 摘要钩子正常返回合法结构化结果', function () {
...
Then('执行硬截断并保留尾部', function () {
    const block = lastRealBlock(this);
    assert.ok(block.text.length > 0, '截断后应保留尾部内容');
});
/* ---- Scenario: 三级全部失败时放弃压缩并告警 ---- */
Given('llm、heuristic、truncate 三级均抛出异常或产出非法结果', function () {
    this.useMemoryArchive();
...
Then('抛出 NoCompressionPossible', function () {
    assert.ok(this.lastError instanceof NoCompressionPossible, `期望 NoCompressionPossible，实际 ${String(this.lastError)}`);
});
Then('记录一条含三级失败原因的告警', function () {
    const error = this.lastError;
    assert.ok(error && error.details !== undefined && Object.keys(error.details).length > 0, '错误应携带三级失败原因');
});
/* ---- Scenario: 每次降级都落盘 method 标记 ---- */
Given('一次压缩最终由 truncate 级完成', function () {
    prepCompressible.call(this);
...
When('检查产出的 CompressedBlock', async function () {
    this.lastEpochBefore = this.state.epoch;
    this.lastCommitted = await this.orchestrator({
        compress: this.hookOverrides.compress,
        shouldCompress: this.hookOverrides.shouldCompress,
...
Then('其 method 字段准确记为 truncate 而非 llm', function () {
    assert.equal(lastRealBlock(this).method, 'truncate');
});
Then('依据该标记可判定此段上下文可信度低于 llm 级摘要', function () {
    assert.equal(lastRealBlock(this).method, 'truncate');
});
Then('标记随报告一并写入观测数据', function () {
...
Then('视为不压缩决策', function () {
    assert.equal(this.lastCommitted, this.state);
});
Then('不因钩子异常而使会话崩溃', function () {
    // 未抛出异常即达此处
    assert.ok(true);
});
/* ---- Scenario: select_segment 钩子抛异常时中止本轮压缩 ---- */
Given('已注入的 select_segment 钩子会抛出异常', function () {
    prepCompressible.call(this);
    this.hookOverrides.shouldCompress = () => true;
...
Then('本轮压缩被中止且上下文完全不变', function () {
    assert.equal(this.lastCommitted, this.state);
});
Then('不回退到任何默认切割实现（边界错则全盘错）', function () {
    assert.ok((this.metrics['select_hook_error'] ?? 0) >= 1);
});
Then('记录一条 select_hook_error 告警', function () {
    assert.ok((this.metrics['select_hook_error'] ?? 0) >= 1);
});
/* ---- Scenario: 用户仅提供 compress 钩子时其余钩子走默认实现 ---- */
Given('注入的 callback 只实现了 compress，其余五个钩子均未提供', function () {
    this.useMemoryArchive();
    this.msgs = fallbackSession();
    this.configOverrides = { ...this.configOverrides, targetBudgetTokens: 1 };
    this.makeConfig();
...
Then('流程可正常跑通不抛缺失实现错误', function () {
    assert.ok(this.lastCommitted !== null);
    assert.notEqual(this.lastCommitted, this.state);
});
Then('触发、切割、校验、降级均由默认实现承担', function () {
    assert.ok((this.lastCommitted?.compressed.length ?? 0) > 0);
});
Then('产出的 CompressedBlock 字段完整可被 rehydrate', function () {
    const block = lastRealBlock(this);
...
//# sourceMappingURL=fallback-chain.steps.js.map