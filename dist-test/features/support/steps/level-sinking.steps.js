...
async function craftTwoL2Blocks() {
...
async function runRoundOn(state) {
...
When('该段需继续下沉至 L3', async function () {
...
function resunkBlockOf() {
    const epoch = this.lastCommitted?.epoch;
    return this.lastCommitted?.compressed.find((b) => b.sourceSpan.startId === this.l2Block?.sourceSpan.startId && b.epoch === epoch);
}
Then('系统经 rehydrate 取回 L0 原文', function () {
...
Then('L3 摘要由 L0 原文生成，输入中不含 L2 摘要文本', function () {
    const round2Inputs = this.compressTexts.slice(this.callsBeforeResink);
    assert.ok(round2Inputs.every((t) => !t.startsWith('摘要块')), '输入不得包含 L2 摘要文本');
});
Then('新 CompressedBlock 的 source_span 指向 L0 原文的消息 ID', function () {
...
Given('一段内容的 compress_count 已等于配置注入的下沉次数上限', async function () {
    const msgs = new HistoryBuilder()
        .system()
        .user('任务')
...
Then('裁剪结果中每个字符片段均可在原文中定位', function () {
...
Then('结果中不存在原文未出现的新字符串', function () {
...
Given('某次工具输出的内容在后续轮次被助手显式引用', function () {
...
When('尝试 rehydrate', async function () {
    try {
...
When('一段内容需从 L1 下沉至 L2', async function () {
...
Then('记录一条 archive-unavailable 告警说明原因是回溯能力缺失', function () {
    assert.ok((this.metrics['archive-unavailable'] ?? 0) >= 1);
});
//# sourceMappingURL=level-sinking.steps.js.map