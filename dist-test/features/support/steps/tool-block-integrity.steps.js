...
Then('中部段的首元素与尾元素都必须是完整 block', function () {
...
Given('一条 assistant 消息含 3 个 tool_calls', function () {
    const calls = [
...
Given('一个 tool block 的 assistant 消息发起了 2 次工具调用', function () {
...
Given('仅 1 条 tool result 到达，另一次调用超时未返回', function () {
    this.pendingMsgs.push(...new HistoryBuilder().tool('call-1', 'r1').build());
});
Then('该 block 标记为 complete 等于 false', function () {
    assert.equal(this.blocks[0]?.complete, false);
});
Then('missing 列表含未返回的 tool_use 标识', function () {
    assert.ok(this.blocks[0]?.missing.includes('call-2'));
});
Then('该 block 被强制 pin 且 pin_reason 为 tool-block-incomplete', function () {
    const block = this.blocks[0];
    assert.ok(this.pins.some((p) => p.reason === 'tool-block-incomplete' && p.msgId === block.startId), '畸形块应被强制 pin');
});
Then('该 block 在任何级别都不参与压缩', function () {
    assert.equal(isBlockCompressible(this.blocks[0]), false);
});
/* ---- Scenario: 孤儿 result 单独成块且不中断流程 ---- */
Given('一条 tool result 消息找不到对应的 assistant tool_calls', function () {
    this.pendingMsgs = new HistoryBuilder().tool('ghost-call', '孤儿结果').build();
});
Then('该 result 单独成块并被标记为畸形', function () {
    assert.equal(this.blocks.length, 1);
    assert.equal(this.blocks[0]?.malformed, true);
});
Then('压缩流程正常继续不抛异常', function () {
    // 解析未抛异常即到达此处；再跑一次 L1 裁剪确认全链路无异常
    assert.ok(Array.isArray(this.blocks));
});
/* ---- Scenario: 无安全切点时放弃本轮压缩 ---- */
Given('中部起点到尾部起点之间的全部消息同属一个 tool block', function () {
...
Then('select_segment 返回空的中部段集合', function () {
    assert.equal(this.segments.length, 0);
});
Then('本轮压缩被放弃且上下文逐字节不变', async function () {
...
Given('三条 tool result 以 call-2、call-3、call-1 的乱序到达', function () {
    this.pendingMsgs.push(...new HistoryBuilder().tool('call-2', 'r2').tool('call-3', 'r3').tool('call-1', 'r1').build());
});
Then('这 4 条消息仍构成 1 个 complete 的 block', function () {
    assert.equal(this.blocks.length, 1);
    assert.equal(this.blocks[0]?.complete, true);
});
Then('block 内消息保持原始到达顺序不被重排', function () {
    assert.deepEqual(this.blocks[0]?.msgIds, this.pendingMsgs.map((m) => m.id));
});
/* ---- Scenario: 被下一条 assistant 工具调用截断的 block 判为畸形 ---- */
Given('一条 assistant 消息发起 2 次工具调用但仅有 1 条 result 到达', function () {
    this.pendingMsgs = new HistoryBuilder()
        .assistant('调用两个', [
        { id: 'call-1', name: 'read', args: {} },
        { id: 'call-2', name: 'read', args: {} },
...
Given('紧随其后是另一条发起工具调用的 assistant 消息', function () {
...
//# sourceMappingURL=tool-block-integrity.steps.js.map